...
with block:
    ...
    with gr.Row():
        gr.Markdown("MIRROR FITS ")
    with gr.Row():
        gr.Markdown("***Support upper-body/lower-body/dresses; garment category must be paired!!!***")
    with gr.Row():
        with gr.Column():
            vton_img_dc = gr.Image(label="Upload Model Image", sources='upload', type="filepath", height=384)
            example = gr.Examples(
        ...
        with gr.Column():
            garm_img_dc = gr.Image(label="Upload Garment Image", sources='upload', type="filepath", height=384)
